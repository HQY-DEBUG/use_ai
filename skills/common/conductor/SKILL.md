...
对每个步骤严格执行以下循环，**不跳步、不合并、不省略任何子步骤**：

```
┌─────────────────────────────────────────┐
│  步骤 N 执行循环                          │
│                                         │
│  1. 预检：读 plan.md 该步骤描述           │
│     ↓                                   │
│  2. 宣告：输出 "▶️ 开始执行步骤N：<标题>"  │
│     ↓                                   │
...
**步骤1：预检（执行前必读）**  
读取 plan.md 中该步骤的完整描述，提取：
- 目标文件路径
- 操作类型（新增/修改/删除）
- 具体要求（功能、接口、约束）
- 依赖条件

这一步的目的是**把 plan 的要求加载进当前上下文**，防止执行时遗忘。
...
**长计划检查点**：步骤数 > 5 时，每完成 3 个步骤暂停一次，调用 `ask_user`：
```
...
每个步骤执行后，**必须**完成以下验证，任一失败则不能标记为 done：
...
**验证2：内容对齐**  
读取刚修改的文件，对照 plan.md 该步骤描述，确认：
- 要求实现的功能/接口是否存在
- 若 plan 要求"修改函数 foo"，则 foo 函数必须有变更
...
- 校验**通过** → 执行 **4.3 原子化提交**，SQL `status = 'done'`
- 校验**失败** → 尝试自动修复（最多 2 次），修复后重新从验证1开始：
  - 修复成功 → 继续

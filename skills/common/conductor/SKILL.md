...
**简单任务** → 跳过阶段 2/3，直接进入阶段 4 执行。  
**复杂任务** → 继续阶段 2 生成完整计划。

> ⚠️ **无论简单还是复杂任务，以下步骤永远不跳过**：
> - **Phase 5.1**：执行后归纳用户习惯，更新 `workspace-rules.md`
> - **Phase 5.3**：输出执行摘要
> - **memory.md**：若本次有架构决策或用户修正，必须追加
>
> "简单任务"仅意味着跳过 plan.md 生成和 SQL 任务表，**不意味着跳过收尾沉淀**。
...
> **执行完成后此行必须立即被 edit 工具更新为**：`- [x] 状态：✅ 已完成（commit: abc1234，YYYY/MM/DD HH:MM）`
...
│     ↓                                               │
│  9. 立即用 edit 工具更新 plan.md 该步骤状态  ← 必须执行 │
│     ↓                                               │
│  10. 进度播报                                         │
└─────────────────────────────────────────────────────┘
```

**步骤9：更新 plan.md（强制，必须调用 edit 工具）**

每个步骤完成后，**立即**（不等全部步骤完成）调用 `edit` 工具修改 `.conductor/plan.md`：
...
改为：
```
- [x] 状态：✅ 已完成（commit: <hash>，YYYY/MM/DD HH:MM）
```

失败时改为：
```
- [x] 状态：❌ 失败（详见 .conductor/errors/step_N_*.log）
```

> ⚠️ **这是强制工具调用，不是可选提示。** 每次完成一个步骤都必须立刻 `edit` plan.md，不得累积到最后再批量更新。批量更新是 plan.md 不同步的根本原因。
...
> ⛔ **本阶段对所有任务（包括简单任务快速路径）均为强制执行，不可跳过。**  
> 简单任务跳过的是 plan.md 和 SQL，但 5.1 习惯归纳、5.3 执行摘要、memory.md 更新始终必须执行。

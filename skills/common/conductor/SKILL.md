...
**第一步：无条件更新 workspace-rules.md 的执行记录**

每次任务结束，无论有无用户修正，都必须用 `edit` 工具在 `.conductor/workspace-rules.md` 的"执行记录"章节追加一行：

```markdown
| YYYY/MM/DD | <本次任务一句话摘要> | <变更的文件列表> |
```

若 workspace-rules.md 不存在，先用 `templates/workspace-rules.md` 初始化创建，再追加。

**第二步：归纳用户修正行为（有则记录，无则跳过）**
...
```
choices: [`[y] 自动写入规则文件`, `[r] 仅记录到 workspace-rules.md`, `[n] 忽略`]

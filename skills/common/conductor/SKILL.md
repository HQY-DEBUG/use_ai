...
> ⚠️ **关键约束：`edit` 工具要求 `old_str` 在文件中唯一。**  
> plan.md 中所有步骤的 `- [ ] 状态：待执行` 完全相同，直接替换这一行会失败或命中错误步骤。  
> **必须将步骤标题行作为上下文**，确保 `old_str` 唯一匹配。

成功时，`old_str` 和 `new_str` 示例：

```
# old_str（包含步骤标题，确保唯一匹配）
...
> ⚠️ 每次完成一个步骤都必须立刻 `edit` plan.md，不得累积到最后再批量更新。
...
### 5.5 跨会话记忆持久化（每次任务结束必须执行）

将本次任务信息写入 `.conductor/memory.md`，**无论有无架构决策都必须执行**。

**操作步骤**：

1. 读取现有 `.conductor/memory.md`（0.1 阶段已确保存在）
2. 用 `edit` 工具追加内容（不覆盖，追加到文件末尾或对应章节）：

**有架构决策时**：在"架构决策记录"表追加一行：
```markdown
| YYYY/MM/DD | <决策内容，如：输入改为索引方式> | <原因> | <影响范围> |
```

**无架构决策时（兜底，必须执行）**：在"执行记录"章节追加一行：
```markdown
| YYYY/MM/DD | <任务摘要> | 无架构变更 |
```

若 memory.md 中没有"执行记录"章节，先追加章节标题：
```markdown
## 执行记录
...
| 日期 | 任务 | 备注 |
|------|------|------|
| 2026/04/15 | 整理代码格式 + 更新仿真接口 | 无架构变更 |
```

> ⚠️ **每次 Phase 5 都必须调用 `edit` 工具写入 memory.md，不得以"无重要决策"为由跳过。** 执行记录章节的兜底写入确保 memory.md 始终反映最新任务历史。

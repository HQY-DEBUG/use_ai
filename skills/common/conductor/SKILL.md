...
若推理出新规则/习惯，用 `edit` 工具追加到"用户习惯推理"表：
```markdown
| YYYY/MM/DD | <规则描述，如：禁止使用 begin 与 always 同行> | 提问记录第3行 | 🔒 强制 | ✅ 已加入生效约束 |
| YYYY/MM/DD | <习惯描述，如：输入倾向使用索引方式> | 提问记录第2、4行 | 中 | ✅ 已加入生效约束 |
```

同时将规则/习惯追加到"生效约束"节（强制规则加 🔒）：
```markdown
- 🔒 禁止使用 begin 与 always 同行（用户明确指令，YYYY/MM/DD）
- 输入倾向使用索引方式（来自用户习惯推理，YYYY/MM/DD）
```

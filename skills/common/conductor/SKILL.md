...
将澄清后的意图拆解为原子任务，生成或覆盖写入 `.conductor/plan.md`（位于工作区根目录）。
...
将本次任务中的关键决策写入 `.conductor/memory.md`（追加模式）。若文件不存在，使用 `templates/memory.md` 初始化后再追加。

> ⚠️ `.conductor/` 目录建议加入 `.gitignore`（用户偏好/决策记录通常不需要进版本控制）。若项目有需要，可选择性提交 `memory.md`。

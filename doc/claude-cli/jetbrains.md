...
如果您还没有安装 Claude Code，请参阅[快速入门指南](https://code.claude.com/docs/zh-CN/quickstart)了解安装说明。

> **注意**：安装插件后，您可能需要完全重启 IDE 才能使其生效。
...
> **提示**：对于 WSL 用户：将 `wsl -d Ubuntu -- bash -lic "claude"` 设置为您的 Claude 命令（将 `Ubuntu` 替换为您的 WSL 发行版名称）
...
> **警告**：使用 JetBrains 远程开发时，您必须通过 **Settings → Plugin (Host)** 在远程主机上安装插件。

插件必须安装在远程主机上，而不是在您的本地客户端计算机上。

### WSL 配置

> **警告**：WSL 用户可能需要额外配置才能使 IDE 检测正常工作。有关详细设置说明，请参阅 [WSL 故障排除指南](https://code.claude.com/docs/zh-CN/troubleshooting#jetbrains-ide-not-detected-on-wsl2)。
...
* 对于 WSL 用户，请参阅 [WSL 故障排除指南](https://code.claude.com/docs/zh-CN/troubleshooting#jetbrains-ide-not-detected-on-wsl2)
...
如需更多帮助，请参阅[故障排除指南](https://code.claude.com/docs/zh-CN/troubleshooting)。

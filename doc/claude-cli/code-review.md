...
> **注意**：Code Review 处于研究预览阶段，仅适用于 Teams 和 Enterprise 订阅。对于启用了 Zero Data Retention 的组织，此功能不可用。
...
要在您自己的 CI 基础设施中运行 Claude 而不是使用此托管服务，请参阅 GitHub Actions 或 GitLab CI/CD。

本页涵盖：

* 审查工作原理
* 设置
* 自定义审查，使用 `CLAUDE.md` 和 `REVIEW.md`
* 定价
...
审查成本随 PR 大小和复杂性而扩展，平均在 20 分钟内完成。管理员可以通过分析仪表板监控审查活动和支出。
...
Code Review 使用对内容的读取访问权限和对 pull request 的写入访问权限。更广泛的权限集也支持 GitHub Actions，如果您稍后启用的话。
...
* **手动**：仅当有人在 PR 上注释 `@claude review` 时才启动审查；之后对该 PR 的后续推送会自动审查
...
Claude 在目录层次结构的每个级别读取 `CLAUDE.md` 文件，因此子目录的 `CLAUDE.md` 中的规则仅适用于该路径下的文件。有关 `CLAUDE.md` 如何工作的更多信息，请参阅内存文档。

对于您不想应用于常规 Claude Code 会话的仅审查指导，请改用 `REVIEW.md`。
...
Code Review 根据令牌使用情况计费。审查平均花费 $15-25，随 PR 大小、代码库复杂性和需要验证的问题数量而扩展。Code Review 使用通过额外使用单独计费，不计入您的计划包含的使用。
...
在任何模式下，注释 `@claude review` 选择 PR 进入推送触发审查，因此在该注释后每次推送都会产生额外成本。
...
通过分析中的每周成本图表或管理员设置中的每个存储库平均成本列监控支出。

## 相关资源

Code Review 旨在与 Claude Code 的其余部分一起工作。如果您想在打开 PR 之前在本地运行审查、需要自托管设置或想深入了解 `CLAUDE.md` 如何在工具中塑造 Claude 的行为，这些页面是很好的下一步：

* [Plugins](https://code.claude.com/docs/zh-CN/discover-plugins)：浏览插件市场，包括用于在推送前本地运行按需审查的 `code-review` 插件
* [GitHub Actions](https://code.claude.com/docs/zh-CN/github-actions)：在您自己的 GitHub Actions 工作流中运行 Claude，以实现超越代码审查的自定义自动化
* [GitLab CI/CD](https://code.claude.com/docs/zh-CN/gitlab-ci-cd)：GitLab 管道的自托管 Claude 集成
* [Memory](https://code.claude.com/docs/zh-CN/memory)：`CLAUDE.md` 文件如何在 Claude Code 中工作
* [Analytics](https://code.claude.com/docs/zh-CN/analytics)：跟踪超越代码审查的 Claude Code 使用情况

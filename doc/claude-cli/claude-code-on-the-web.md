...
> **注意**：Claude Code on the web 目前处于研究预览阶段。
...
你可以从终端使用 `--remote` 在网络上启动新任务，或将网络会话传送回你的终端以在本地继续。要在运行 Claude Code 在自己的机器上而不是云基础设施上时使用网络界面，请参阅[远程控制](https://code.claude.com/docs/zh-CN/remote-control)。
...
2. **环境设置**：Claude 准备一个安全的云环境，包含你的代码，然后运行你的设置脚本（如果已配置）
...
> **注意**：会话切换是单向的：你可以将网络会话拉入你的终端，但不能将现有的终端会话推送到网络。`--remote` 标志为你的当前存储库创建一个**新的**网络会话。
...
使用 `/tasks` 监控所有会话。当会话完成时，你可以从网络界面创建 PR 或传送会话到你的终端以继续工作。
...
对于 Enterprise 和 Teams 账户，两个可见性选项是**私有**和**团队**。团队可见性使会话对你的 Claude.ai 组织的其他成员可见。默认情况下启用存储库访问验证，基于连接到收件人账户的 GitHub 账户。你的账户显示名称对所有有访问权限的收件人可见。Claude in Slack 会话会自动以团队可见性共享。
...
1. **环境准备**：我们克隆你的存储库并运行任何已配置的设置脚本。存储库将使用你的 GitHub 存储库上的默认分支进行克隆。如果你想检出特定分支，可以在提示中指定。
...
> **注意**：Claude 完全通过环境中可用的终端和 CLI 工具运行。它使用通用镜像中的预装工具和你通过 hooks 或依赖管理安装的任何其他工具。

**要添加新环境**：选择当前环境以打开环境选择器，然后选择"添加环境"。这将打开一个对话框，你可以在其中指定环境名称、网络访问级别、环境变量和设置脚本。
...
设置脚本是一个 Bash 脚本，在新的云会话启动时运行，在 Claude Code 启动之前。使用设置脚本来安装依赖、配置工具或准备云环境需要的任何东西，这些东西不在默认镜像中。

脚本在 Ubuntu 24.04 上以 root 身份运行，所以 `apt install` 和大多数语言包管理器都可以工作。

> **提示**：要在将其添加到你的脚本之前检查已安装的内容，请要求 Claude 在云会话中运行 `check-tools`。
...
> **注意**：安装包的设置脚本需要网络访问才能到达注册表。默认网络访问允许连接到常见包注册表，包括 npm、PyPI、RubyGems 和 crates.io。如果你的环境禁用了网络访问，脚本将无法安装包。

#### 设置脚本与 SessionStart hooks

使用设置脚本来安装云需要但你的笔记本电脑已有的东西，如语言运行时或 CLI 工具。使用 SessionStart hook 进行应该在任何地方运行的项目设置，云和本地，如 `npm install`。
...
自定义环境镜像和快照尚不支持。使用设置脚本在会话启动时安装包，或使用 SessionStart hooks 进行也应在本地环境中运行的依赖安装。
...
* **需要网络访问**：安装命令需要网络访问才能到达包注册表。如果你的环境配置为"无互联网"访问，这些 hooks 将失败。使用"受限"（默认）或"完全"网络访问。默认允许列表包括常见的注册表，如 npm、PyPI、RubyGems 和 crates.io。
* **代理兼容性**：远程环境中的所有出站流量都通过安全代理。某些包管理器不能与此代理正确配合使用。Bun 是一个已知的例子。
...
默认情况下，网络访问仅限于允许列表域。你可以配置自定义网络访问，包括禁用网络访问。

### 默认允许的域

使用"受限"网络访问时，默认允许以下主要域类别：

* **Anthropic 服务**：api.anthropic.com、claude.ai 等
* **版本控制**：github.com、gitlab.com、bitbucket.org 及其 API
* **容器注册表**：docker.io、gcr.io、ghcr.io、mcr.microsoft.com、public.ecr.aws 等
* **云平台**：Google Cloud、Azure、AWS、Oracle Cloud 相关域
* **包管理器 - JavaScript/Node**：registry.npmjs.org、yarnpkg.com 等
* **包管理器 - Python**：pypi.org、files.pythonhosted.org 等
* **包管理器 - Ruby**：rubygems.org 及相关域
* **包管理器 - Rust**：crates.io、rustup.rs 等
* **包管理器 - Go**：proxy.golang.org、pkg.go.dev 等
* **包管理器 - JVM**：maven.org、gradle.org、kotlin.org 等
* **包管理器 - 其他语言**：packagist.org（PHP）、nuget.org（.NET）、pub.dev（Dart）、hex.pm（Elixir）等
* **Linux 发行版**：archive.ubuntu.com、ppa.launchpad.net 等
* **开发工具**：Kubernetes、HashiCorp、Anaconda、Apache、Eclipse、Node.js 相关域
* **监控服务**：statsig.com、sentry.io、datadoghq.com 等
* **Model Context Protocol**：\*.modelcontextprotocol.io

> **注意**：标记为 `*` 的域表示通配符子域匹配。例如，`*.gcr.io` 允许访问 `gcr.io` 的任何子域。
...
> **注意**：在禁用网络访问的情况下运行时，Claude Code 被允许与 Anthropic API 通信，这可能仍然允许数据从隔离的 Claude Code VM 中退出。
...
1. **自动化环境设置**：使用设置脚本在 Claude Code 启动之前安装依赖和配置工具。对于更高级的场景，配置 SessionStart hooks。

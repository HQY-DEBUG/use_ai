...
本节定义标准仓库的顶层目录结构与各子目录用途，AI 在新建文件或建议路径时须遵循此约定。
...
- 新建文件时，根据文件类型放入对应目录，不得随意放置在根目录
- `doc/<库名>/` 目录名与所描述的库或模块名称一致，不使用通用名称
- `PL/source/` 下可按功能再分子目录，但保持 `constraints/`、`ip/`、`sim/`、`verilog/` 四个标准子目录
